...
describe('retry policy', () => {
  ...
  it('backs off increasingly on PRODUCT_NOT_READY then gives up bounded', async () => {
    const sleeps: number[] = [];
    let calls = 0;
    const client = clientWithStub(
      {
    ...
    await expect(client.syncTransactions('tok', null, 500)).rejects.toThrow(/PRODUCT_NOT_READY/);
    // Bounded at MAX_ATTEMPTS: 4 calls, 3 sleeps, never unbounded.
    expect(calls).toBe(4);
    expect(sleeps).toEqual([3000, 6000, 9000]);
  });
